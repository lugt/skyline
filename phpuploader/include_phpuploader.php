...
if(@$_GET["ContextValue"])
{
	...
	{
		echo("无法设置session id 请在考前位置设置session_start()");
		exit(200);
	}
}
...
function PhpUploader_MoveUploadedFile($_file,$_line,$src,$dst)
{
	...
	if(!PhpUploader_FileExists($src))
		throw(new Exception("文件不存在 : $src , at $_file line $_line"));
	...
	{
		$le=error_get_last();
		throw(new Exception($le["message"] . " , 移动文件从 $src 到 $dst' , 在 $_file 行号 $_line"));
	}
	return $re;
}
...
function PhpUploader_MakeDir($_file,$_line,$dir,$flag)
{
	...
	{
		$le=error_get_last();
		throw(new Exception($le["message"] . " , 无法创建文件夹 '$dir' , 在 $_file 行号 $_line"));
	}
	return $re;
}
function PhpUploader_Copy($_file,$_line,$src,$dst)
{
	...
	{
		$le=error_get_last();
		throw(new Exception($le["message"] . " , 无法拷贝 $src 到 $dst' , 在 $_file 行号 $_line"));
	}
	return $re;
}
function PhpUploader_Move($_file,$_line,$src,$dst)
{
	...
	{
		$le=error_get_last();
		throw(new Exception($le["message"] . " , 无法移动文件  $src  到  $dst' , at $_file line $_line"));
	}
	return $re;
}
...
function PhpUploader_Delete($_file,$_line,$file)
{
	...
	{
		$le=error_get_last();
		throw(new Exception($le["message"] . " , 无法删除  $file' , at $_file line $_line"));
	}
	return $re;
}
function PhpUploader_FileOpen($_file,$_line,$filepath,$flag)
{
	...
	{
		$le=error_get_last();
		throw(new Exception($le["message"] . " , 无法打开  $filepath' , at $_file line $_line"));
	}
	return $re;
}
...
function PhpUploader_GetGuid($str)
{
	if(!preg_match("/^[0-9A-F]{8}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{4}-[0-9A-F]{12}$/i",$str))
		throw(new Exception("无效的 Guid : " . $str));
	return $str;
}
...
function PhpUploader_GetSystemTempFolder()
{
	...
	else
	{
		if(!is_dir($str))
		{
			...
			if(!$result)
			{
				throw(new Exception("文件夹 $str 并不存在 .  请检查权限 or specify a temp folder using TempDirectory property."));
			}
		}
	}
	...
	if (!is_dir($str))
	{
		...
		if(!$result)
		{
			throw(new Exception("无法创建临时文件夹: $str  请检查权限 or specify a temp folder using TempDirectory property."));
		}
	}
...
class PhpUploader
{
	...
	function GetString()
	{
		...
		if($this->SaveDirectory)
		{
			if(!$this->AllowedFileExtensions)
			{
				...
				$code.="当使用 SaveDirectory property 时, 你必须指定 AllowedFileExtensions 以确保安全.";
				$code.="</div>";
			}
		}
		...
		{
			$code.="<button id='" . $this->Name . "Button' class='btn' onclick='return false;'>" . $this->InsertText . "</button>";
		}
	...
	function LoadSecuritySettings()
	{
		...
		{
			throw (new Exception("$this->Name : 回话已过期! 请刷新再试."));
		}
	}
	...
	function _MoveHttpFile($file,$guid)
	{
		...
		if(!$result)
		{
			throw(new Exception("无法保存文件到  $filepath"));
		}
	}
